...
// ─── Account config ────────────────────────────────────────────────────────────
const ACCOUNTS = [
  { id: "3191", acct: "02920020000003191", label: "JDB — 02920020000003191 (ບັນຊີຈ່າຍ)" },
  { id: "3180", acct: "02920020000003180", label: "JDB — 02920020000003180 (ບັນຊີຮັບ)" },
] as const;
type AcctId = typeof ACCOUNTS[number]["id"];
...
export interface JdbReconRow {
  ...
  "ລວມມີ":              string;
  // DR cols
  "SPLUS_PRICE":        string;   // ລາງວັນ Sokxay (BANK_DR)
  "FEE_SPLUS_PRICE":    string;   // ຄ່າທໍານຽມ Sokxay (BANK_DR)
  "SPLUS_PRO":          string;   // ໂຊກຊ້ອນໂຊກ (BANK_DR)
  "SPLUS_REFUND":       string;   // Refund Sokxay (BANK_DR)
  "FEE_SPLUS_REFUND":   string;   // ຄ່າທໍານຽມ Refund (BANK_DR)
  "LOTTO_SELL":         string;   // ຂາຍຫວຍ (BANK_CR)
  "TRANSFER":           string;   // ໂອນ-ໜີ້ (BANK_DR)
  "TRANSFER_CR":        string;   // ໂອນ-ມີ (BANK_CR)
  "ATT":                string;   // ATT (BANK_DR)
  "IBANK_FEE":          string;   // iBank Fee (BANK_DR)
  "FTR_FEE":            string;   // FTR Fee (BANK_DR)
  "TRANSFER_FEE":       string;   // Transfer Fee (BANK_DR)
  "FEE_JDB_LOTTO_SETTL":string;  // Fee JDB Lotto Settl (BANK_DR)
  "SAVING_INTEREST":    string;   // Saving Interest (BANK_CR)
  "SPLUS_PRICE_TAX":    string;   // ອາກອນ 5% (BANK_CR)
  "ອື່ນໆ":              string | null;
  "ສ່ວນຕ່າງ":           string;
}
...
const COLS: { key: ColKey; label: string; align?: "left" | "right" | "center"; dir?: "dr" | "cr" }[] = [
  { key: "ວັນທີ",            label: "ວັນທີ",                  align: "center" },
  { key: "ລວມໜີ້",           label: "ລວມໜີ້",                 align: "right",  dir: "dr" },
  ...
  { key: "TRANSFER",         label: "ໂອນ-ໜີ້",                align: "right",  dir: "dr" },
...
const PRINT_CSS = `
  @media print {
    ...
    * { font-family: 'Phetsarath OT', 'Phetsarath', sans-serif !important; color: #000 !important; box-shadow: none !important; text-shadow: none !important; border-radius: 0 !important; -webkit-print-color-adjust: exact; print-color-adjust: exact; }
    ...
    tbody tr td, tbody tr.bg-amber-50 td, tbody tr.bg-amber-100 td, tbody tr.hover\\:bg-amber-100 td, tbody tr.hover\\:bg-blue-50 td { background: #fff !important; background-color: #fff !important; }
...
// ─── Component ────────────────────────────────────────────────────────────────
export default function JdbBankReconciliationPage() {
  const { user } = useAuth();

  const [activeAcct, setActiveAcct] = useState<AcctId>("3191");

  const [dateFrom, setDateFrom] = useState("");
  const [dateTo, setDateTo] = useState("");
  const [rowsByAcct, setRowsByAcct] = useState<Record<AcctId, JdbReconRow[]>>({ "3191": [], "3180": [] });
    ...
    try {
      const results = await Promise.all(
        ACCOUNTS.map(async ({ acct, id }) => {
          ...
          return { id, rows: Array.isArray(json.rows) ? json.rows : [] };
        })
      );
      const map = { "3191": [] as JdbReconRow[], "3180": [] as JdbReconRow[] };
      results.forEach(({ id, rows }) => { map[id as AcctId] = rows; });
      setRowsByAcct(map);
    } catch (e: unknown) {
  ...
  // ── Derived ────────────────────────────────────────────────────────────────
  const rows     = rowsByAcct[activeAcct];
  ...
  const acctInfo = ACCOUNTS.find((a) => a.id === activeAcct)!;
  ...
  return (
    <>
      ...
      <div className="print-area flex flex-col gap-4">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h1 className="text-base font-semibold text-slate-800">
                Bank Reconciliation JDB
              </h1>
              <p className="text-xs text-slate-400">JDB_STMT · ຈຳແນກຕາມວັນທີ (BANK_TXN_DATE)</p>
            </div>
          </div>
        ...
        {/* ── Account Tabs ────────────────────────────────────────────────── */}
        <div className="no-print flex gap-0 border border-slate-300 rounded-xl overflow-hidden w-fit">
          {ACCOUNTS.map((a) => (
            <button
              key={a.id}
        ...
        {hasData && stats && (
          <div className="no-print grid grid-cols-3 gap-3">
            ...
            <div className={`border rounded-xl p-3 flex items-center gap-3 ${Math.abs(stats.diff) < 0.01 ? "bg-slate-50 border-slate-200" : "bg-amber-50 border-amber-300"}`}>
              ...
              <div>
                <p className={`text-xs font-medium ${Math.abs(stats.diff) < 0.01 ? "text-slate-500" : "text-amber-600"}`}>ສ່ວນຕ່າງ (Diff)</p>
                <p className={`text-sm font-bold font-mono ${Math.abs(stats.diff) < 0.01 ? "text-slate-700" : "text-amber-700"}`}>
                  {fmt(stats.diff)}
                  {Math.abs(stats.diff) < 0.01 && (
                    <span className="ml-1 text-xs text-emerald-600" style={{ fontFamily: "'Noto Sans Lao', 'Inter', sans-serif", fontWeight: "normal" }}>✓ ສົມດູນ</span>
                  )}
                </p>
              </div>
            </div>
        ...
        <div className="hidden print:block mb-2" style={{ textAlign: "center" }}>
          ...
          <h1 style={{ fontSize: "13px", fontWeight: "bold", margin: "4px 0 0 0" }}>
            Bank Reconciliation JDB — {acctInfo.acct}
          </h1>
                ...
                <tbody>
                      ...
                      >
                        ...
                        {COLS.map((col) => {
                          ...
                          if (col.key === "ລວມມີ" && display)
                            return <td key={String(col.key)} className={TD + " text-emerald-700"}>{display}</td>;
                          if ((col.dir === "cr") && display)
                  ...
                  {totalRow && (
                    <tr className="total-row bg-gray-200 font-bold">
                      <td className={TDC + " bg-gray-200 font-bold"} colSpan={2}
