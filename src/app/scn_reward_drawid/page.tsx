...
import {
  ...
  AlertCircle, BarChart3, Printer, X, Filter, Gift,
} from "lucide-react";
import { useAuth } from "@/lib/authContext";
import PageSkeleton from "@/components/PageSkeleton";
import { logActivity } from "@/lib/activityService";

interface RewardDrawRow {
  DRAWID: number;
  DRAW_DATE: string;
  TT_TXN: number;
...
type SortKey = keyof RewardDrawRow;
...
function groupByMonth(rows: RewardDrawRow[]) {
  const map = new Map<string, RewardDrawRow[]>();
  for (const r of rows) {
    const raw = r.DRAW_DATE ?? "";
...
export default function ScnRewardDrawidPage() {
  ...
  const [rows,        setRows]        = useState<RewardDrawRow[]>([]);
  ...
  const fetchRows = async (from: string, to: string) => {
    ...
    setRowsError(null);
    try {
      const qs = new URLSearchParams({ view: "reward_drawid" });
  ...
  const handleApplyFilter = () => {
    ...
    if (user) logActivity({ uid: user.uid, displayName: user.displayName, email: user.email, action: "reward_search", detail: `Reward DRAWID: ${drawFrom || ""}~${drawTo || ""} ${search || ""}`.trim() });
  };
  ...
  const filtered = useMemo(() => {
    ...
    const data = rows.filter(r => {
      if (q && !(String(r.DRAWID ?? "").includes(q) || r.DRAW_DATE?.toLowerCase().includes(q))) return false;
      return true;
    });
  ...
  const handlePrint = () => {
    if (user) logActivity({ uid: user.uid, displayName: user.displayName, email: user.email, action: "reward_print", detail: `Reward DRAWID: ${appliedFrom || ""}~${appliedTo || ""} (${filtered.length} ງວດ)` });
    window.print();
  };
...
.overflow-x-auto { overflow: visible !important; }
  ...
  const TH = "px-2 py-2 text-center font-bold text-slate-700 cursor-pointer hover:text-amber-600 select-none whitespace-nowrap bg-yellow-100 border border-black text-[11px]";
      ...
      <div className="print-area flex flex-col gap-4">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 no-print">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h1 className="text-base font-semibold text-slate-800">ລາຍງານ ການຈ່າຍລາງວັນ ຫວຍ SCN - ສັງລວມເປັນງວດ</h1>
              <p className="text-xs text-slate-400">APP_V_SCN_REWARD_DRAWID</p>
            </div>
          </div>
        ...
        {/* Filter box */}
        <div className="no-print bg-amber-50 border border-amber-200 rounded-xl p-4 flex flex-wrap items-end gap-3">
          ...
          <div className="flex flex-col gap-1">
            ...
            <select value={drawFrom} onChange={e => setDrawFrom(e.target.value)}
              className="px-3 py-2 text-sm border border-black rounded-lg focus:outline-none focus:ring-2 focus:ring-amber-500 bg-white w-52 disabled:opacity-50"
        ...
        <div className="hidden print:block mb-2" style={{ textAlign: "center" }}>
          <h1 style={{ fontSize: "16px", fontWeight: "bold", margin: 0 }}>ລາຍງານ ການຈ່າຍລາງວັນ ຫວຍ SCN - ສັງລວມເປັນງວດ</h1>
          {filterSummary && (
        ...
        <div className="bg-white border border-black rounded-xl overflow-hidden">
          {loadingIds || loadingRows ? (
            ...
            <div className="flex flex-col items-center justify-center py-20 gap-3 text-slate-400 no-print">
              ...
              <p className="text-sm">ເລືອກງວດແລ້ວກົດ <span className="font-semibold text-amber-600">ສະແດງຂໍ້ມູນ</span></p>
            </div>
          ) : (
              ...
              <table className="w-full text-xs border-collapse">
                <thead>
                  <tr className="bg-yellow-100">
                    {([
                      ["DRAWID", "ງວດ"], ["DRAW_DATE", "ວັນທີ"], ["TT_TXN", "ຈໍານວນລາຍການ"],
                ...
                <tbody>
                  {filtered.length === 0 ? (
                    <tr><td colSpan={10} className="text-center py-16 text-slate-400">
                      ...
                      <p>{rows.length > 0 ? `ບໍ່ມີຂໍ້ມູນທີ່ຕົງກັບ "${appliedSearch}"` : "ບໍ່ມີຂໍ້ມູນໃນຊ່ວງງວດທີ່ເລືອກ"}</p>
                    </td></tr>
                  ) : (
                    <>
                      {Array.from(grouped.entries()).map(([monthKey, gRows]) => {
                        ...
                        return (
                          <React.Fragment key={monthKey}>
                            {gRows.map((r, i) => (
                              <tr key={`${monthKey}-${i}`} className="hover:bg-amber-50">
                      ...
                      <tr className="grandtotal-row bg-gray-400 font-bold text-sm">
                        <td className="px-2 py-2 border border-black" colSpan={2}>ລວມທັງໝົດ</td>
