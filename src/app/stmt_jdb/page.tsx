...
import { useGet_Reward_JDB_STMT } from "@/app/CASES-LOTTO/hooks/hook_JDB";
...
function PageContent() {
  ...
  const { data, isLoading, isError, refetch, isFetching } =
    useGet_Reward_JDB_STMT(billNumber);
      ...
      <div className="min-h-screen bg-slate-50 p-4 md:p-8">
        <div className="w-full max-w-7xl mx-auto space-y-6">
          ...
          <div className="flex items-center justify-center gap-3 rounded-2xl border border-blue-100 bg-blue-50/50 p-4 text-sm font-medium text-blue-700">
            ...
            ກຳລັງໂຫຼດຂໍ້ມູນ Statement JDB... (ອາດໃຊ້ເວລາປະມານ 1-4 ນາທີ)
          </div>
          <Skeleton />
        </div>
      </div>
    ...
    <div className="min-h-screen bg-slate-50 text-slate-800 antialiased p-4 md:p-8 selection:bg-blue-100">
      <div className="mx-auto max-w-[90rem] space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <button
            onClick={() => router.back()}
            className="group inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-semibold text-slate-600 shadow-sm transition hover:text-slate-900 hover:border-slate-300 self-start"
          >
            <ArrowLeft
              size={16}
            ...
            <div className="space-y-1">
              <div className="flex flex-wrap items-center gap-3">
                ...
                <h1 className="text-xl md:text-2xl font-bold tracking-tight text-slate-900">
                  ລາຍງານ Statement JDB
                </h1>
              </div>
              <p className="text-sm text-slate-500 pl-12">
                ກວດສອບລາຍລະອຽດການເຄື່ອນໄຫວທຸລະກຳ ແລະ ການຈ່າຍເງິນລາງວັນ JDB
              </p>
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="rounded-2xl border border-slate-200/70 bg-white p-6 shadow-sm lg:col-span-2 space-y-1">
            ...
            <Row
              ...
              value={data!.LOTTO_BILL_NO || billNumber || "-"}
            />
...
export default function REWARD_JDB_STMT() {
  return (
    <Suspense
      fallback={
