        ...
        <input
          ...
          className={`flex-1 rounded-xl border px-4 py-2.5 text-sm text-slate-800 outline-none transition focus:ring-2 ${
            error
              ...
              : "border-slate-200 focus:ring-blue-200 hover:border-slate-300"
          }`}
        />
        <button
          onClick={onSearch}
          disabled={isFetching}
          className="inline-flex items-center gap-2 rounded-xl bg-blue-600 px-5 py-2.5 text-sm font-semibold text-white transition hover:bg-blue-700 active:scale-95 disabled:opacity-50"
        >
          {isFetching ? (
...
function PageContent() {
  ...
  // ── search state ──────────────────────────────────────────────────────────
  const caseNumberFromUrl = searchParams.get("case_number") ?? "";
  const [inputValue, setInputValue] = useState(caseNumberFromUrl);
  const [inputError, setInputError] = useState("");

  // caseNumber drives the query — only updates when user confirms search
  const [caseNumber, setCaseNumber] = useState(caseNumberFromUrl);

  const { data, isLoading, isError, refetch, isFetching } =
    useGet_refund_points(caseNumber);

  const refunds: REFUND_POINTS_ENTITY[] = Array.isArray(data)
    ? data
    : data
      ? [data]
      : [];
  ...
  const handleSearch = () => {
    ...
    if (!trimmed) {
      setInputError("ກະລຸນາໃສ່ເລກເຄສກ່ອນຄົ້ນຫາ");
      return;
    }
    setInputError("");
    setCaseNumber(trimmed); // triggers re-fetch
    const params = new URLSearchParams(searchParams.toString());
    params.set("case_number", trimmed);
    router.replace(`?${params.toString()}`); // sync URL
  };
      ...
      <div className="min-h-screen bg-slate-50 p-4 md:p-8">
        <div className="mx-auto max-w-7xl space-y-6">
          ...
          <div className="flex flex-col items-center justify-center rounded-2xl border border-red-100 bg-white p-10 shadow-xl ring-1 ring-red-50">
            ...
            <h2 className="text-xl font-bold text-slate-800">ບໍ່ພົບຂໍ້ມູນ</h2>
            <p className="mt-2 text-sm text-slate-500 max-w-md text-center">
              ບໍ່ພົບຂໍ້ມູນສຳລັບ{" "}
              <span className="font-semibold text-slate-700">{caseNumber}</span>{" "}
              ກະລຸນາກວດສອບ ຫຼື ລອງໃໝ່ອີກຄັ້ງ.
            </p>
            <button
              onClick={() => refetch()}
              disabled={isFetching}
              className="mt-6 inline-flex items-center gap-2 rounded-xl bg-blue-600 px-6 py-3 text-sm font-semibold text-white shadow-md transition-all hover:bg-blue-700 active:scale-95 disabled:opacity-50"
            >
              <RefreshCw
                size={16}
