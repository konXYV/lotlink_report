...
export default function IssueModal({ issue, maxNo, previewCode, existingCategories = [], onSave, onClose }: Props) {
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-xl max-h-[92vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-semibold text-slate-600 mb-1.5 uppercase tracking-wide">ລະຫັດ Issue</label>
              {issue ? (
                <div className="w-full border border-slate-200 rounded-xl px-3 py-2 text-sm bg-slate-100 text-slate-500 font-mono cursor-not-allowed select-none">
                  {issue.issueCode || `#${issue.no}`}
                </div>
              ) : (
                <div className="w-full border border-dashed border-blue-300 rounded-xl px-3 py-2 text-sm bg-blue-50 text-blue-600 font-mono cursor-not-allowed select-none flex items-center gap-2">
                  <span className="text-slate-400 text-xs">🎲</span>
                  <span>{previewCode || "ISS-????"}</span>
                </div>
              )}
            </div>
            <div>
